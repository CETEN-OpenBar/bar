...
func (b *Backend) GetAllStarrings(ctx context.Context, page uint64, size uint64, startAt, endAt uint64) ([]*models.Starring, error) {
	ctx, cancel := b.TimeoutContext(ctx)
	defer cancel()
...
func (b *Backend) CountAllStarrings(ctx context.Context, startAt, endAt uint64) (uint64, error) {
	ctx, cancel := b.TimeoutContext(ctx)
	defer cancel()
