<script context="module" lang="ts">
	import type { TransactionNotification } from './transactionNotifications.svelte';

	export type TransactionAction = 'finish' | 'cancel' | 'restart' | 'save';
	export type TransactionFeedback = TransactionNotification & {
...
<script lang="ts">
	...
	const customer = {
		customerName: transaction.account_name.trim(),
		customerNickname: transaction.account_nick_name?.trim()
	};
	...
	const successMessages: Record<TransactionAction, string> = {
		finish: 'Transaction validée',
		cancel: 'Transaction annulée',
		restart: 'Transaction remise en attente',
		save: 'Modifications enregistrées'
	};
	const errorMessages: Record<TransactionAction, string> = {
		finish: 'Validation impossible',
		cancel: 'Annulation impossible',
		restart: 'Remise en attente impossible',
	...
	async function performAction(action: TransactionAction) {
		...
		try {
			if (action === 'save' || action === 'finish') {
				for (const [index, item] of draft.items.entries()) {
					// Re-sending an amount for a canceled item restores its cost on the server.
					if (originalItems[index].state === TransactionItemState.TransactionItemCanceled) continue;
		...
		notify({ ...customer, kind: 'success', action, message: successMessages[action] });
		pending = false;
	}
