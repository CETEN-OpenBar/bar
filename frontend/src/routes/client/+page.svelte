<script lang="ts">
	// import Carousel from '$lib/components/client/carousel.svelte';
	...
	import FsLoading from '$lib/components/client/fs_loading.svelte';
	...
	function pinCallback(pin: string) {
		...
		authApi()
			...
			.then((res) => {
				if (res.data.account?.state == AccountState.AccountOK) goto('/client/commande');
				if (res.data.account?.state == AccountState.AccountNotOnBoarded) goto('/client/onboarding');
			})
			.catch(() => {
