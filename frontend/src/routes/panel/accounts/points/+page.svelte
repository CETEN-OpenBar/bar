<script lang="ts">
	...
	async function updatePoint() {
		...
		let oldPointNumber = selectedAccount.points;
		let res = await accountsApi().addPoint(selectedAccount.id, amount, { withCredentials: true });
		if (res.status == 200){
			selectedAccount.points = oldPointNumber+amount;
		}
		}
	...
	<div class="w-[50%] flex flex-col gap-5">
		...
		{#if selectedAccount}
			<div class="bg-slate-100 flex flex-col rounded-xl border-2 p-4">
					...
					{#if selectedAccount?.points}
					<div class="text-3xl text-center font-bold p-5">
						{selectedAccount?.first_name} {selectedAccount?.last_name}</div>
						<div class="text-3xl text-center font-bold p-5">

						Nombre de points : {selectedAccount?.points}
